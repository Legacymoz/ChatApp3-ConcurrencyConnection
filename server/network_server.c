...
static int determine_lan_ip(char *ip_buffer, size_t ip_buffer_size) {
    ...
    while (host_info->h_addr_list[index] != NULL) {
        ...
        if (converted != NULL &&
            ...
            strncmp(converted, "172.30.", 7)      != 0) {  /* skip WSL        */
...
static int setup_discovery_socket() {
    ...
    if (discovery_socket == INVALID_SOCKET) return 0;
    ...
    if (bind(discovery_socket, (struct sockaddr *)&discovery_addr,
             sizeof(discovery_addr)) == SOCKET_ERROR) {
...
int initialize_server_network() {
    ...
    if (get_tailscale_status(status_output, sizeof(status_output),
        ...
        printf("[Network] Tailscale unavailable or offline. Using LAN mode.\n");
    }
...
SOCKET get_discovery_socket()    { return discovery_socket; }
...
void handle_discovery_request(SOCKET socket_handle) {
    ...
    int received = recvfrom(socket_handle, buffer, BUFFER_SIZE - 1, 0,
                            (struct sockaddr *)&client_addr, &client_addr_len);
    if (received <= 0) return;

    buffer[received] = '\0';
    if (strcmp(buffer, DISCOVERY_REQUEST) != 0) return;

    snprintf(response, sizeof(response), "%s|%s|%d",
             DISCOVERY_RESPONSE_PREFIX, local_lan_ip, PORT);

    sendto(socket_handle, response, strlen(response), 0,
           (struct sockaddr *)&client_addr, client_addr_len);
